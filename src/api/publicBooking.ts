 ...
 * время появилось.
 *
 * Выложен на прод 10.09.2026 (флаг `WAITLIST_PUBLIC_CHANNEL_ENABLED`). Заявка
 * не материализуется в `Patient` и не занимает слот: имя и телефон гостя
 * ложатся в `contactName`/`phone`, `patientId` остаётся пустым, пока
 * регистратор не свяжет запись с картой.
 *
 * ⚠ Ошибки этой ручки приходят в старом плоском виде публичного API
 * (`{error: "validation_error", message, details: {patient_phone: "…"}}`), а не
 * в конверте `error.code/details.fields` приватного: `extractErrorMessage`
 * показывает `message` («Некорректный номер телефона.»), пофайловый разбор
 * `details` к этой форме не применяется — на форму из трёх полей хватает.
 * Анти-спама на ручке нет (§9.7 тикета остаётся открытым),
 * поэтому ошибка пробрасывается как есть и разбирается на стороне UI.
 */
