 ...
 *
 * ⚠ Флаг выключен: на бэке эндпоинта ещё нет. Пока `false`, шаблоны хранятся
 * в localStorage браузера (см. `localDriver` ниже) — это даёт рабочий UI для
 * демонстрации и правки макетов, но шаблон виден только на том же устройстве
 * и не переживает чистку хранилища. Контракт согласуется тикетом
 * `MamaDoc/backend_ticket_conclusion_form_templates.md`; после его реализации
 * переключаем флаг — вызовы ниже уже ходят по согласованным путям.
 */
export const CONCLUSION_FORMS_BACKEND = false;
