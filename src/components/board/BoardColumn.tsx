...
export interface BoardColumnProps {
  /** Идентификатор для dnd-kit (`col:<id>`): цель броска в пустую колонку и ниже списка. */
  dndId: string;
  title: string;
  dotColor?: string;
  count?: number;
...
const BoardColumn: React.FC<BoardColumnProps> = ({
  ...
  // Зона броска — вся колонка (и шапка тоже): промахнуться мимо списка,
  // отпустив над заголовком, не должно быть возможно.
  const { setNodeRef } = useDroppable({ id: dndId, disabled: !droppable });
      ...
      >
        {loading ? (
          Array.from({ length: 3 }).map((_, k) => (
            <Skeleton key={k} variant="rounded" height={92} />
          ))
        ) : empty ? (
