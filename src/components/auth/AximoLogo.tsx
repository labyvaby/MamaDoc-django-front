...
const AximoLogo: React.FC<Props> = ({ light = false, compact = false, size, iconOnly = false }) => {
  const color = light ? "#ffffff" : "#159c91";
  const iconSize = size ?? (compact ? 30 : 36);

  return (
    <Stack direction="row" alignItems="center" gap={compact ? 1 : 1.25}>
      <Box component="svg" viewBox="0 0 40 40" aria-hidden sx={{ width: iconSize, height: iconSize, flexShrink: 0, color }}>
