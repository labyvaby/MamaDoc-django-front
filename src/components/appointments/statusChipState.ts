...
export function getStatusChipState(
   ...
   * зелёного у нала). Баланс, бонусы и страховка сюда не входят и остаются в
   * общем зелёном «Оплачено»: они не про то, легли ли деньги в ящик, а про
   * источник средств, и у страховки есть отдельный чип. Смешанная оплата
   * (нал + карта) — тоже общий зелёный: в списке приходят только способы,
   * без сумм по каждому, поэтому какой из них главный, здесь не определить.
   */
