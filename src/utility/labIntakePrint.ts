 ...
 *
 * Этикетки и памятка собираются разом из одного и того же чека
 * (`LabReceipt`) — дровер зовёт эту функцию один раз и получает готовый
 * HTML для кнопок печати (lab-frontend-design.md, раздел «Печать»).
 * Регистрационный лист сюда не входит: ему нужна карточка заказа с
 * составом и суммами (`labRegistrationSheet.ts`), а чек её не несёт. Сама печать
...
import { buildLabelsHtml, buildPreparationHtml, type LabelData } from "./labLabels";
