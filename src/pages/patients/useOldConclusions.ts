/**
 * useOldConclusions — вся история заключений пациента: архив прошлых систем
 * плюс живые записи текущей.
 *
 * Восстановлено 19.08.2026 после чистки legacy-модулей (3b4400f), удалившей
 * хук вместе со страницей patient-search. Прежняя supabase-ветка при этом
 * выброшена: клиент удалён той же чисткой, режим остался один — Django.
 */
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        setErrorMsg(null);
        setData(await fetchConclusions(patientPhone, patientId, ctrl.signal));
      } catch (err: unknown) {
