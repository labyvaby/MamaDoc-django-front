...
type Props = {
  ...
  loading: boolean;
  error: string | null;
  /** Кнопка «Добавить» контакт — clients.create. */
  canCreate: boolean;
  /** Кнопка «Изменить» контакт — clients.update. */
  canUpdate: boolean;
...
export default function ClientTabs({ tab, purchases, contacts, loading, error, canCreate, canUpdate, canViewPurchases, onAddContact, onEditContact }: Props) {
  if (loading) return <BoxCenter><CircularProgress size={28} /></BoxCenter>;
  if (error) return <Alert severity="error">{error}</Alert>;
  if (tab === "contacts") {
    return <Card sx={{ height: "100%", overflowY: "auto" }}><Stack direction="row" justifyContent="space-between" alignItems="center" p={2}><Typography variant="h6">Контактные лица</Typography>{canCreate && <Button startIcon={<AddOutlined />} onClick={onAddContact}>Добавить</Button>}</Stack><Divider /><List>{(contacts ?? []).map((contact) => <ListItem key={contact.id ?? `self-${contact.fullName}`} secondaryAction={canUpdate && contact.id ? <Button size="small" onClick={() => onEditContact(contact)}>Изменить</Button> : undefined}><ListItemText primary={contact.fullName} secondary={[contact.position, contact.phone, contact.email].filter(Boolean).join(" · ") || "Контакты не указаны"} /><Chip size="small" label={contact.isPrimary ? "Основной" : "Контакт"} sx={{ mr: canUpdate && contact.id ? 10 : 0 }} /></ListItem>)}{!contacts?.length && <Typography color="text.secondary" sx={{ p: 2 }}>Контактных лиц пока нет.</Typography>}</List></Card>;
  }
