    ...
    >
      ...
      <Box sx={{ px: theme.appLayout.page.paddingX, pt: 0, pb: 1.5 }}>
        <Stack direction="row" alignItems="center" gap={1.5} flexWrap="wrap" useFlexGap>
          ...
          {canManage && !isMobile && tab === "calendar" && !vivaActive && (
            <Button
              size="small"
              variant="contained"
      ...
      {/* Телефон: действия вкладки в плавающей кнопке. На «Настройке» — выбор
          «график / отсутствие», в календаре — сразу смена. На Viva календарь —
          шахматка броней, смену там не добавляют (как и кнопкой в шапке). */}
      {isMobile && canManage && !(vivaActive && tab === "calendar") && (
        <>
          <Fab
            color="primary"
