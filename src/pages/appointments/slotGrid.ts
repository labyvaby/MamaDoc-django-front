 ...
 *
 * Шаг живёт в карточке сотрудника (`employee.slotDurationMinutes`). С 10.09.2026
 * бэк применяет его сам в `availability` — но только в запросе без `serviceId`
 * (у услуги приоритет её длительность), и не на всех стендах сразу. Пересборка
 * здесь оставлена страховкой: где шаг уже применён, она ничего не делает —
 * нарезка уже нарезанного тем же шагом даёт те же окна (тест «идемпотентность»).
 * Вызывать её можно только там, где услуга в запрос не передаётся, иначе она
 * перебьёт сетку по длительности услуги.
 */
