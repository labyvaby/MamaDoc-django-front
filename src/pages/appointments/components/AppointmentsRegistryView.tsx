 ...
 *
 * Фильтр «Оплачено» зелёный, а строки под ним бывают и бирюзовыми: способ
 * оплаты (нал/безнал) с 05.08.2026 различается цветом чипа, но фильтр здесь —
 * по статусу оплаты, и оба способа для него один статус. Это не рассинхрон
 * палитры, а разная гранулярность: фильтра по способу в реестре нет.
 */
const PAYMENT_CHIPS: {
  value: PaymentFilter;
